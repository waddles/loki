...
func (s *store) init() error {
	for i, p := range s.schemaCfg.Configs {
		...
		tableRange := p.GetIndexTableNumberRange(periodEndTime)
		w, idx, stop, err := s.storeForPeriod(p, tableRange, chunkClient, f)
		if err != nil {
			return err
		}
...
func (s *store) storeForPeriod(p config.PeriodConfig, tableRange config.TableRange, chunkClient client.Client, f *fetcher.Fetcher) (stores.ChunkWriter, index.ReaderWriter, func(), error) {
	...
	if p.IndexType == config.TSDBType {
		...
		tableStart := p.From.Unix() / int64(p.IndexTables.Period/time.Second)
		prefix := fmt.Sprintf("%s_%d", p.ObjectType, tableStart)
